...
> [!TIP]
> Apache Hop graduated in December 2021.
...
Mentors and community members are encouraged to contribute to this page and comment on it, the following table summarizes project’s self-assessment against the [Apache Maturity Model](http://community.apache.org/apache-way/apache-project-maturity-model.html).
...
| CD20 | Anyone can easily discover and access the project’s code. | The Apache Hop (Incubating) source code is available on [GitBot](https://gitbox.apache.org/repos/asf?p=hop.git) and [github](http://github.com/apache/hop) |
| CD30 | Anyone using standard, widely-available tools, can build the code in a reproducible way. | The Apache Hop (Incubating) is built after each commit and integration tests are run on a daily basis on the [ASF Jenkins server](https://ci-builds.apache.org/job/Hop/). + Development environment setup and build instructions are [documented](http://hop.apache.org/dev-manual/latest/setup-dev-environment.html) on the Hop website. |
| CD40 | The full history of the project’s code is available via a source code control system, in a way that allows anyone to recreate any released version. | The entire commit history is available from the [beginning](https://github.com/apache-hop/commit/e24b53ea7dfe6bcad9e03011d1ce88eeb1bb7edc). |
...
| --- | --- | --- |
| ID | Description | Status |
| RE10 | Releases consist of source code, distributed using standard and open archive formats that are expected to stay readable in the long term. | Releases are available via https://dlcdn.apache.org/incubator/hop/. Source code and convenience binaries are linked to from the [website](http://hop.apache.org/download/). |
...
| RE30 | Releases are signed and/or distributed along with digests that anyone can reliably use to validate the downloaded archives. | Releases are signed, the keys are available from the [Hop download page](http://hop.apache.org/download/). |
| RE40 | The project can distribute convenience binaries alongside source code, but they are not Apache Releases, they are provided with no guarantee. | Convenience binaries are available from the download page, a Docker image is available from [Dockerbub](https://hub.docker.com/r/apache/hop) |
| RE50 | The project documents a repeatable release process so that someone new to the project can independently generate the complete set of artifacts required for a release. | The processes to [create](https://hop.apache.org/dev-manual/latest/apache-release/creating-a-release.html), [promote](https://hop.apache.org/dev-manual/latest/apache-release/promoting-a-release.html) and [check](https://hop.apache.org/dev-manual/latest/apache-release/checking-a-release.html) a release are documented. |
...
| QU20 | The project puts a very high priority on producing secure software. | Security issues are treated with the highest priority. We use Apaches default way to submit security related information, which is described on the [website](https://issues.apache.org/security/). |
...
| CO50 | The project documents how contributors can earn more rights such as commit access or decision power, and applies these principles consistently. | The process to become a committer or (P)PMC member is documented on the [website](https://hop.apache.org/community/contributing/). |
...
| --- | --- | --- |
| ID | Description | Status |
| CS10 | The project maintains a public list of its contributors who have decision power. The project’s PMC (Project Management Committee) consists of those contributors. | Committers and contributors are listed on the website’s [team page](https://hop.apache.org/community/team/). |
